...
if ( row != null )
{
len = Math . Round ( Convert . ToDecimal ( row [ "JDB007" ] . ToString ( ) ) );
...
{
    if ( tableViewTwo . Rows . Count == len )
    {
        gridControl2 . DataSource = tableViewTwo;
        return;
    }
    ...
    {
        _model . STM006 = getMaxCodeForSame ( _model . STM009 ,_model . STM010 );
        if ( string . IsNullOrEmpty ( _model . STM006 ) )
        {
            len = len - tableViewTwo . Rows . Count;
            if ( _model . STM012 . Substring ( 0 ,1 ) . Equals ( "0" ) )
            {
                _model . STM012 = "1" + _model . STM012;
            }
            //_model . STM012 = ( Convert . ToInt64 ( _model . STM012 ) + 1 ) . ToString ( );
            for ( long i = 0 ; i < len ; i++ )
            {
                _model . STM006 = ( Convert . ToInt64 ( _model . STM012 ) + i ) . ToString ( );
                if ( _model . STM012 . Substring ( 0 ,1 ) . Equals ( "1" ) )
                {
                    _model . STM006 = _model . STM006 . Substring ( 1 ,_model . STM006 . Length - 1 );
                }
                addRowTo ( );
            }

            tableList . Add ( _model . STM009 + _model . STM010 ,_model . STM006 );
        }
        else
        {
            _model . STM012 = _model . STM006;
            if ( _model . STM012 . Substring ( 0 ,1 ) . Equals ( "0" ) )
            {
                _model . STM012 = "1" + _model . STM012;
            }
    ...
    {
        if ( tableList . ContainsKey ( _model . STM009 + _model . STM010 ) )
        {
            _model . STM012 = ( string ) tableList [ _model . STM009 + _model . STM010 ];
            if ( _model . STM012 . Substring ( 0 ,1 ) . Equals ( "0" ) )
            {
                _model . STM012 = "1" + _model . STM012;
            }
            ...
            tableList . Remove ( _model . STM009 + _model . STM010 );
            tableList . Add ( _model . STM009 + _model . STM010 ,_model . STM006 );
